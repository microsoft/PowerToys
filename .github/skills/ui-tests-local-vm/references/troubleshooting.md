...
| Symptom | Boundary | Action |
|---|---|---|
| `New-PSSession -VMName` fails with a logon error | Control account/DPAPI file | First distinguish an expired/disabled guest account from a mismatched or unreadable credential. If the saved credential is wrong, recreate it with `Get-Credential \| Export-Clixml`, typed directly into the prompt. DPAPI files do not roam between host users or machines. |
| PowerShell Direct, `runas`, or UAC rejects the configured administrator because its password expired | Guest administrator password expiry | A human must restore console access first if PowerShell Direct cannot authenticate. Then rerun `Initialize-LocalVmHost.ps1` (not `-CheckOnly`) to set only that configured account's `PasswordNeverExpires` flag; no password reset, account enabling, privilege grant, or machine-wide policy change is performed. Refresh again after restoring an old checkpoint, or deliberately replace the verified clean baseline. See [setup.md](setup.md#disposable-guest-account-password-expiry). |
