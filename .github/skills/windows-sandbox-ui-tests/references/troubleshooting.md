...
|---|---|---|
| Black window, connection lost, no `progress.json` | Guest desktop never logged on | Stop the exact environment. Launch through Start-menu AppID, wait for `ExistingLogin`, then share dynamically. Do not blame product/tests. |
| `0x80070520` from `ExistingLogin` | No active guest user session | Wait after Start activation. Avoid split `wsb start` + `wsb connect` and pre-login large mappings. |
