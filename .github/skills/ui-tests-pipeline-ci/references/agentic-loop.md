...
| `AzureDevOpsExtension` | `az pipelines runs artifact download` is available for pipeline artifacts. |
...
| `PipelinePreview` | The Run Pipeline API accepts the identity, branch, parameters, and template expansion. `id=-1` proves no build was created. This is the strongest safe setup check for the queue path. |
...
### First-time setup and failure remediation
