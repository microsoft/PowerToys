...
if ($CheckAll) {
    ...
    if (Test-Path $reviewDir) {
        $issuesToCheck = Get-ChildItem -Path $reviewDir -Directory | ForEach-Object { [int]$_.Name }
    }
...
if ($JsonOutput) {
    ...
    if ($results.Issues.Count -gt 0) {
        ...
        Write-Host ("-" * 100)
        Write-Host ("{0,-8} {1,-8} {2,-8} {3,-5} {4,-5} {5,-10} {6,-8} {7,-8}" -f "Issue", "Review", "Plan", "Feas", "Clar", "Worktree", "Commits", "PR")
        Write-Host ("-" * 100)
        foreach ($issue in $results.Issues | Sort-Object IssueNumber) {
            ...
            Write-Host ("{0,-8} {1,-8} {2,-8} {3,-5} {4,-5} {5,-10} {6,-8} {7}" -f 
                "#$($issue.IssueNumber)", $reviewMark, $planMark, $issue.FeasibilityScore, $issue.ClarityScore, $wtMark, $commitMark, $prMark)
        }
    }
    ...
    if ($results.PRs.Count -gt 0) {
        ...
        Write-Host ("-" * 120)
        Write-Host ("{0,-8} {1,-10} {2,-10} {3,-8} {4,-8} {5,-10} {6,-12} {7,-10}" -f "PR", "State", "Issue", "Reviews", "High", "Medium", "Comments", "Unresolved")
        Write-Host ("-" * 120)
        foreach ($pr in $results.PRs | Sort-Object PRNumber) {
            ...
            Write-Host ("{0,-8} {1,-10} {2,-10} {3,-8} {4,-8} {5,-10} {6,-12} {7,-10}" -f 
                "#$($pr.PRNumber)", $pr.State, $issueMark, $reviewMark, $pr.HighSeverityCount, $pr.MediumSeverityCount, $pr.ActiveCommentCount, $pr.UnresolvedThreadCount)
        }
    }
