...
description: Review community bug-fix PRs with 7-dimension code review, build verification, and verification guide generation. Use when asked to review a community PR, check a bug fix PR, verify a PR builds, generate review comments, or audit a community contribution.
license: Complete terms in LICENSE.txt
---

# Community PR Review Skill

**Review** and **build-verify** community bug-fix PRs. Produces GitHub-ready review comments, a build report, and end-to-end verification instructions.
...
├── scripts/
│   ├── Start-CommunityPRReview.ps1       # Main orchestrator
...
Review the code changes across:
...
Each finding includes: file, line range, severity, dimension, and actionable suggestion.

### Phase 3: Build Verification
1. Checkout the PR branch in the worktree
2. Run `tools/build/build-essentials.cmd` and `tools/build/build.cmd`
3. On failure:
   - Read error log (`build.*.errors.log`)
   - Try merging latest main (`git merge origin/main`)
...
| File | Description |
|------|-------------|
| `review-comments.md` | All review comments in GitHub markdown, ready to post |
...
```json
{
  "status": "success",
  "prNumber": 45234,
  "reviewFindings": { "high": 0, "medium": 2, "low": 1, "info": 3 },
  "buildStatus": "success",
  "buildActions": [],
...
1. Fetch PR data with GitHub MCP tools
2. Review across all 7 dimensions
3. Write `review-comments.md` with findings
