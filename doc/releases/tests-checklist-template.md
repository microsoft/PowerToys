...
* PowerRename can extract metadata (e.g., EXIF and XMP) from photos **new**
...
Cursor Wrap: **new**
 - [ ] Ensure Cursor wrap is disabled by default upon upgrade.
 - [ ] Turn on cursor wrap and test if it can work without error.
 - [ ] Test in multi screen to see if it can work without error.
 ...
 * Test the different settings and verify they are applied:
  ...
  * Hosts file retention configuration  **new**
   - [ ] Configure hosts file retention settings and make sure it works
...
* Peek should be able to launch through CLI **new**
   - [ ] Can use peek command to peek files
   - [ ] Peek can work without problem when a peek session is on
 ...
 * Test Settings:
  ...
  * Horizonton scroll **new**
   - [ ] Verify horizontol scrolling is working as expected in MWB session
