...
3. **UI test (written, not yet run locally).** Mouse Button Lock is in the `MouseUtils.UITests` restart scope (`FindMyMouseTests.cs`) and now has a dedicated `MouseButtonLockTests.cs` covering enable/disable, the three button checkboxes, the hold-duration slider, and the drag threshold. The shared `MouseUtilsSettings` helper carries the module (enum entry, accessibility ids, name maps), and the "Buttons and behavior" expander gained `MouseUtils_MouseButtonLockOptionsId` so the test can expand it before its items are realized. The test project builds clean, but the test has **not** been executed locally: it needs WinAppDriver (v1.2.1, installed by `.pipelines/InstallWinAppDriver.ps1`), which is not present in this environment, so CI is the first real run. If it fails there, the likely causes are the checkbox strings matched by display text or the expander needing realization before its items are findable. The engine itself is covered by `MouseButtonLock.UnitTests`.
