...
**Right-button caveat (context menu).** Releasing a right-button lock necessarily emits a right-button-up, which applications answer with a context menu (that is just what a right-click is). To keep hands-free right-drag usable, the module injects an `Esc` immediately after the release up to dismiss that menu, but only when the release is not chorded: a same-button release tap and the lifecycle/settings releases (module disable, a button's lock turned off), where no other button is physically down. A cross-button release (pressing a different button to free the held one) injects the up while the new button is down; no `Esc` is sent there, so the dismissal keystroke cannot leak to the foreground app and close a dialog or cancel an operation. Normal quick right-clicks never lock and are untouched, so context menus still work; only a lock *release* auto-dismisses. The trade-off is that a genuine right-drag-drop menu (e.g. Explorer's copy/move) is also dismissed.
...
- **Deferred injection (critical):** `SendInput` is **not** called synchronously inside the hook callback. Doing so lets the very event being suppressed leak to applications (it reads as a stray click that collapses a text selection). Instead `WinInjector` posts a `WM_MOUSEBUTTONLOCK_INJECT` thread message back to the hook thread; the message loop performs the `SendInput` after the callback has returned and the physical event is fully suppressed. On shutdown the target thread is cleared so `ReleaseAll`'s injections run inline (the loop is gone by then, and we are no longer inside a callback, so a direct `SendInput` is safe).
- **Context-menu dismiss:** after injecting a right-button up (always a lock release), `WinInjector` injects an `Esc` so the context menu the up would open is dismissed, but only when the engine flags the release as non-chorded (`IButtonUpInjector`'s `dismissContextMenu`: true for same-button and lifecycle/settings releases, false for a cross-button release). The intent bit rides in bit 0 of the deferred `WM_MOUSEBUTTONLOCK_INJECT` message's `wParam`, with the button in bits 1+ (see the right-button caveat under Behavior).
...
Build status: build- and runtime-verified. The module builds under Visual Studio 2026 with the .NET 10 toolset the repo now targets (Visual Studio 2022 / MSBuild 17.x cannot build the .NET 10 managed projects; use the VS 2026 / MSBuild 18 toolset), linking `PowerToys.MouseButtonLock.dll` (exporting `powertoy_create`), and all 17 `MouseButtonLock.UnitTests` pass. At runtime the built runner loads the module via `knownModules`, the Settings UI shows the controls, and the lock latches/releases correctly in real apps: a text selection made during a hands-free left-drag survives release, any button press frees a held button, and a right-button hands-free drag releases without leaving a context menu (all verified live in Notepad and Chrome).

## Runtime verification

The unit tests cover the Win32-free `Engine`. The Win32 glue in `dllmain.cpp` (hook install, the message pump, the deferred `WM_MOUSEBUTTONLOCK_INJECT` injection, suppression of the up by returning `1`, the `Esc` context-menu dismiss, and the injection-tag self-filter) was verified live against the running runner with temporary tagged trace logging in `dllmain.cpp` (since removed):
...
- The RMB "left-click gets stuck" report traced to the lock only releasing on a same-button tap, so other clicks passed through but the still-held right button blocked them. Fixed by releasing every held button on any button-down. At the time, the `Esc` injected after every right release cleared the context menu this produced; verified live in Notepad and Chrome. **Since changed:** review feedback pointed out the blanket `Esc` can leak to the foreground app when no menu opens, so a cross-button release (which is chorded with the newly pressed button) no longer sends `Esc`; only same-button and lifecycle/settings releases dismiss. A fresh live pass after that change confirmed no stray menu is left open on a cross-button release, so the earlier observation did not reproduce against the current implementation.

That verification used a throwaway console harness that drove the hook with `SendInput` and polled `GetAsyncKeyState`. It was never tracked in the repo: it was built ad hoc into the gitignored `x64\Release\` output and is gone after a clean, so the runs described above are not reproducible from a fresh clone. Anyone repeating this work should recreate it, or promote it to a tracked `tools\` project per the [tools convention](../../tools/readme.md) (PowerToys keeps standalone debug/test apps under `tools\`, built into `{install}\tools`), with a short page under `doc\devdocs\tools\` and an entry in that readme. The core of that check (`SendInput` presses against the running runner, then polling `GetAsyncKeyState` for a swallowed release) is now reproducible from the tracked `HeldButtonLocksPastThresholdAndReleasesOnTap` UI test in `MouseButtonLockSettingsTests.cs`; the trace-logging checks of the message pump, the deferred injection, and the `Esc` dismiss are not.
...
- Local run: the ASan toolchain is available via the "C++ AddressSanitizer" VS component. The recorded local run compiled the target with `/fsanitize=address /fsanitize=fuzzer` directly through cl.exe (bypassing the repo's vcpkg-gated msbuild), with the ASan runtime DLL copied beside the binary; it executed 815,727 inputs in 26 seconds with zero crashes and no ASan findings. That build script was ad hoc in the gitignored `x64\Release\` output and is not tracked in the repo, so reproduce it with the cl.exe invocation above rather than by looking for a checked-in script.
...
Done: GPO is fully wired end to end (`gpo.h` constant + getter, GPOWrapper `idl`/`h`/`cpp`, both `ModuleGpoHelper`s, and the ADMX/ADML templates in `src/gpo/assets/`, where the `ConfigureEnabledUtilityMouseButtonLock` policy targets `SUPPORTED_POWERTOYS_0_102_0`, the first release that can ship the module); ESRP signing lists `PowerToys.MouseButtonLock.dll`; and the spell-check dictionary has the new tokens. The in-process DLL is harvested into the installer by the existing `$(Platform)\Release\*.dll` glob (like the other MouseUtils DLLs), so no per-module `.wxs` is needed. A 36x36 settings/dashboard icon (`MouseButtonLock.png`) and a C++ unit-test project (`MouseButtonLock.UnitTests`, 17 tests over the state machine, including left-button coverage) are in place; the test runs in CI via the solution's `Build;Test` target, so no pipeline edit is needed. The left (primary) mouse button is supported alongside right and middle (off by default; see the Settings section), and the fuzz target exercises all three buttons.

Also done, from a later pass auditing the module against the sibling MouseUtils utilities: DSC v3 support (registered in `SettingsResource.cs`, so the build emits `microsoft.powertoys.MouseButtonLock.settings.dsc.resource.json`; reference page under `doc/dsc/modules/`, plus the overview/settings-resource/App lists and the winget examples); a 192x192 module icon in `doc/images/icons/`, drawn to match the siblings rather than upscaled from the 36x36 settings glyph; an OOBE entry on the Mouse Utilities page; the module's GPO state in the bug report (`ReportGPOValues.cpp`), which every sibling emits and this one did not; `PowerToys.MouseButtonLock.dll` in the installer verification script's signed-file list; and a `getConfiguredMouseButtonLockEnabledValue` case in `Gpo.Tests.cpp`.
...
2. **Command Palette toggle (decided: skip).** Every sibling MouseUtils module is toggleable from the Command Palette, but those commands fire a module **activation/trigger event** (e.g. `ToggleCursorWrapCommand` -> `Constants.CursorWrapTriggerEvent()`, which the CursorWrap module listens for in order to run its activation-hotkey logic). Mouse Button Lock has no activation hotkey and no toggle action (activation is the physical hold), and `ModuleEnablementService` is read-only, so there is nothing to mirror cleanly. Settings are already reachable from the palette via the generic "Open in Settings" MouseUtils command, so no per-module command is added. The only per-module command that would be meaningful is a "release currently held buttons" quick-action, which would require a new trigger event in the shared interop layer (`shared_constants.h` + `Constants.idl`/`.h`/`.cpp`, an ABI-sensitive change) plus a module-side listener; left as an optional future feature, not a parity gap.

3. **UI tests (written, not yet run end to end).** `MouseButtonLockSettingsTests.cs` lives in the migrated `MouseUtils.UITests.Next` suite, modeled on `AutoHideCursorSettingsTests.cs`; there is no legacy `MouseUtils.UITests` copy. Like the rest of the .Next suite it asserts effects rather than control round-trips:
    - The "Buttons and behavior" expander is disabled with the module and enabled with it.
    - The three lock checkboxes persist independently to `settings.json` and survive a `RestartScope()`.
    - Hold duration (200 and 2200 ms) and move-cancel distance (0 and 100 px) persist at their bounds and survive a restart.
    - A held button locks past the configured duration and releases on a same-button tap; a release under the threshold, or a hold on a button whose lock is off, does not lock.

    The module runs inside the runner with no worker process, window, or named event, so lock state is read with `GetAsyncKeyState`: a locked button keeps reading as down because the hook swallows its release. The hook only skips its own `INJECTION_TAG` events, so `SendInput` presses from `MouseHelper` exercise the real engine. The gestures use the left and middle buttons, because a right-button release that does not lock opens the desktop context menu. Settings applies enable/disable through the runner asynchronously, so the enable/disable checks retry for up to 10 s, and cleanup taps only a button that still reads as down, with Settings minimized. Both test projects build clean; the suite has not been run end to end yet (it drives the real desktop and restarts PowerToys, so use a pipeline run or the local-VM flow in `.github/skills/ui-tests-local-vm`). Coverage is tracked in `src/modules/MouseUtils/MouseUtils.UITests/Release-Test-Checklist-Migration-Progress.md`. The engine itself is covered by `MouseButtonLock.UnitTests`.
...
Cross-checks against the `AGENTS.md` validation checklist: no third-party dependencies were added (no `NOTICE.md` change needed); the settings schema is mirrored on both sides (C# `MouseButtonLockProperties` and C++ `parse_settings`); the work from this pass (the `CheckMoveCancel` hardening, the fuzz project, the telemetry edits, the UI tests, and this doc) is committed on `feature/mouse-button-lock`; keep the PR atomic and link the proposal issue microsoft/PowerToys#48302.
