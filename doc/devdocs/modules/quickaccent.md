...
- **The width is measured, never derived from the character count.** An accent cell is a `MinWidth` of 48 DIP, not a fixed 48: a glyph wider than that (₹, ‰, ﷼, a CJK fallback) grows its cell. `SelectorControl.MeasureContentWidthDip` measures the list against an unbounded width and `Calculation.GetToolbarWidth` floors that at one minimum cell per character, applies the description row's minimum and caps it at the display's usable width. A count-based estimate makes the window narrower than its own content, and the list then scrolls silently inside it. The bar is sized **twice** per summon: once before `Show`, and again after the first real layout pass, because the first measurement runs while the surface is still `Collapsed` (and, on the first summon of the process, before its template has ever been applied) and can therefore report less than the items need. The correction happens while the bar is still transparent, so it is never seen as a resize.
- **The window is shown before its content exists, so the content is unveiled separately.** A hidden WinUI 3 window renders nothing, which means the HWND becomes visible while the freshly rebuilt bar has never been laid out. `MainWindow` therefore shows the bar with `Selector.Opacity = 0` — [the documented way to keep an element in layout while it is not painted](https://learn.microsoft.com/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.uielement.visibility), unlike `Visibility.Collapsed`, which stops it communicating a desired size — lays it out, and unveils it a couple of frames later on `CompositionTarget.Rendering`. Attaching a `Rendering` handler [forces the UI thread to run every frame](https://learn.microsoft.com/windows/windows-app-sdk/api/winrt/microsoft.ui.xaml.media.compositiontarget.rendering), so those frames do arrive; the timeout that backs them up is there because the cadence carries no guarantee and can stop for a locked or occluded session. This is the WinUI 3 counterpart of the WPF implementation, which rendered the toolbar off screen and then moved it into view.
