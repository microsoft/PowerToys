...
The windows are created as C++ windows and the window sizes are set to default by scaling them as per DPI using the `DPIAware::Convert` API from common lib. (INSERTCODELINK). Since the UI is launched on a new thread, the window may not be in the foreground, so we call `SetForegroundWindow`(INSERTCODELINK).

`DesktopWindowXamlSource` has to be declared and it is initialized using the `XamlBridge`(INSERTCODELINK), and a second window handle is generated for the internal Xaml Island window. Most of the code was based on the Xaml Island Sample (https://github.com/microsoft/Xaml-Islands-Samples/blob/master/Samples/Win32/SampleCppApp/XamlBridge.cpp INSERTLINK). The `XamlBridge` class contains code which handles initializing the Xaml Island containers as well as handling special messages like keyboard navigation, and focus between islands and between the C++ window and the island. It also has methods for clearing the xaml islands and closing the window.

Once the UI controls are created, the parent container is set as the content for the `DesktopWindowXamlSource` and the `XamlBridge.MessageLoop` is executed. Messages are processed by the C++ window handler like `EditKeyboardWindowProc` (INSERTCODELINK). The general structure we use for this is, for any `WM_PAINT` or `WM_SIZE` message we resize the Xaml Island window. For `WM_GETMINMAXINFO` we set minimum widths so that the window cannot be resized beyond a minimum height and width. This is done to prevent the WinUI elements from overlapping and getting cropped. If it is neither of these cases we send the message to the `XamlBridge.MessageHandler` which handles Destroy, Activation and Focus. If `WM_NCDESTROY` is received when the `XamlBridge` is `nullptr` we close the window thread.
...
Clicking the Type Button opens a content dialog which registers key delays using the `KeyDelay` class (INSERTLINK) for Enter and Esc keys and sets the UI states such that when a key event occurs the TextBlocks on the ContentDialog are updated accordingly. On accepting the dialog the selected keys are copied into the ComboBoxes from the TextBlocks, and on closing the window the key delays are unregistered and UI states are reset. (INSERTCODELINK).
...
`ValidateAndUpdateKeyBufferElement` does not reference any UI components and instead takes all the relevant data as arguments. This method has tests at (INSERTCODELINK) which covers all the cases that could arise from making selections on the UI.

### Shortcut ComboBox Selection Handler
On making a selection in the drop down, the selection handler validates the input with the buffer from the other column and other rows. Error messages are shown using flyouts if the selection is not considered valid and the drop down and buffer for that entry are reset to empty selection.
This differs from the Single Key ComboBox handler in the sense that after validating the current selection we may perform a set of action such as:
...
- Shortcut can only have upto 2 modifiers (Ctrl+Shift+Alt is not supported as we have enforced a 3 key constraint (**not a backend limitation**))
...
`ValidateShortcutBufferElement` does not reference any UI components and instead takes all the relevant data as arguments. This method has tests at (INSERTCODELINK) which covers all the cases that could arise from making selections on the UI.

**Note:** After updating the buffer we have code to handle a special case, which was required to prevent scenarios where a drop down can get deleted but the corresponding `KeyDropDownControl` object isn't deleted. The code checks if the drop down is still linked to the parent and accordingly deletes the `KeyDropDownControl` object from the vector.(INSERTCODELINK)

**IgnoreKeyToShortcutWarning special case:** An additional check was added to ignore the Map to Same key errors when an existing remapping is loaded. This was because a remapping like Ctrl->Ctrl+A has an intermediate step of Ctrl->Ctrl, which could lead to an error of invalid input, even though Ctrl+A is valid. The only way to actually add this is from the Type button or by adding them in a different order (like typing Shift+A and then changing Shift to Ctrl). Since the intermediate check could fail, this was causing the app to crash since the Xaml Island wouldn't be completely loaded at that point and the Flyout can't be displayed. (INSERTLINK https://github.com/microsoft/PowerToys/issues/6695)