...
- [GetSingleKeyRemapResult](#GetSingleKeyRemapResult)
- [HandleSingleKeyRemapEvent](#HandleSingleKeyRemapEvent)
- [HandleShortcutRemapEvent](#HandleShortcutRemapEvent)
- [HandleOSLevelShortcutRemapEvent](#HandleOSLevelShortcutRemapEvent)
- [HandleAppSpecificShortcutRemapEvent](#HandleAppSpecificShortcutRemapEvent)
- [HandleSingleKeyToggleToModEvent (Obsolete))](#HandleSingleKeyToggleToModEvent-(Obsolete---Code-from-PoC-which-is-commented-out))
- [Tests](#Tests)
  - [MockedInput](#MockedInput)
  - [Tests for single key remaps and shortcut remaps](#Tests-for-single-key-remaps-and-shortcut-remaps)

## GetSingleKeyRemapResult

`State::GetSingleKeyRemapResult` gets a sequence of remapped key events to be injected as well as a flag indicating the original key event should be suppressed or not.

- Check if the current key is pressed while a dual-mapped key is pressed. If it is, inject key events for the combination key remap of the dual-mapped key. For example, if Space key (`VK_SPACE`) is dual-mapped to Left Shift key (`VK_LSHIFT`) as the combination key remap, pressing 'A' key while Space key is pressed should inject a `VK_LSHIFT` key down before processing 'A' key press event. Then, forget about the dual-mapped key.
- Check if the current key is dual-mapped, meaning that the key has either or both of the alone remap (effective only when the key is pressed alone) and combination remap (effective only when the key is pressed together with other keys).
- If the current key is dual-mapped and being pressed, just record the current key into the state. It's because we cannot tell which remap to be used at this point.
- If the current key is dual-mapped and being released, check whether other keys have been pressed while the key is pressed. This can be done by examining whether the pressed dual-mapped key has been forgotten or not.
  - If other keys have been pressed, inject a key up event for the combination remap.
  - If other keys haven't been pressed, inject a pair of key up and down events for the alone remap.
- If the current key is remapped but not dual-mapped, inject a key down or up event using the remapped key.
- If the current key is not remapped, do nothing.
- The original key event should be suppressed when the key is remapped.

## HandleSingleKeyRemapEvent

`KeyboardEventHandlers::HandleSingleKeyRemapEvent` is used for handling the key to key and key to shortcut remapping logic. The general logic is as follows:
...
- For each remapped key event returned by `State::GetSingleKeyRemapResult`, do the followings:
  - If it is remapped to Disable, suppress the event.
  - If it is remapped to a key, we send the key down/up message for the target key and suppress the current key event. We have a check for filtering artificial keys, such as `VK_WIN` (which is a key code added by us), so that it is translated to `VK_LWIN` instead.
...
In order to test the remapping logic, a mocked keyboard input handler had to be created because otherwise the tests would process and send actual key events. For this the [`InputInterface`](https://github.com/microsoft/PowerToys/blob/main/src/modules/keyboardmanager/common/InputInterface.h) was made, and in production code the methods are implemented using [`SendInput`](https://learn.microsoft.com/en-us/windows/win32/api/winuser/nf-winuser-sendinput) and [`GetAsyncKeyState`](https://learn.microsoft.com/en-us/windows/win32/api/winuser/nf-winuser-getasynckeystate). In addition to this, [`GetCurrentApplication`](https://github.com/microsoft/PowerToys/blob/b80578b1b9a4b24c9945bddac33c771204280107/src/modules/keyboardmanager/common/Helpers.cpp#L226-L268) had to be mocked so that app-specific remapping can be tested.
