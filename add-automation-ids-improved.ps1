...
function Add-AutomationIds {
    ...
    # This handles both single-line and multi-line elements
    $pattern = '(?s)(<[^>]*?\s+x:Uid="([^"]+)"[^>]*?)(?![^>]*AutomationProperties\.AutomationId)([^>]*>)'
    ...
    $newContent = [regex]::Replace($content, $pattern, {
        ...
        # Check if we're in a multi-line element by looking for newlines
        if ($beforeUid -match '\r?\n' -or $afterUid -match '\r?\n') {
            # Multi-line element - add on new line with proper indentation
            $replacement = $beforeUid + "`r`n    AutomationProperties.AutomationId=`"$uidValue`"" + $afterUid
        } else {
            # Single-line element - add inline
            $replacement = $beforeUid + " AutomationProperties.AutomationId=`"$uidValue`"" + $afterUid
        }
