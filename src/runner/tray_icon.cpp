...
void open_shortcuts_flyout_window(const POINT flyout_position)
{
...
void handle_tray_command(HWND window, const WPARAM command_id, LPARAM lparam)
{
    ...
    case ID_SHOW_SHORTCUTS_MENU_COMMAND:
    {
        POINT mouse_pointer;
        GetCursorPos(&mouse_pointer);
        open_shortcuts_flyout_window(mouse_pointer);
        break;
    }
    }
}
...
void click_timer_elapsed()
{
    ...
    if (!double_clicked)
    {
        open_shortcuts_flyout_window(tray_icon_click_point);
    }
}
        ...
        if (message == wm_icon_notify)
        {
            ...
            case WM_CONTEXTMENU:
            {
                ...
                if (h_menu)
                {
                    ...
                    static std::wstring show_shortcuts_menuitem_label = GET_RESOURCE_STRING(IDS_SHOW_SHORTCUTS_MENU_TEXT);
                    ...
                    change_menu_item_text(ID_SHOW_SHORTCUTS_MENU_COMMAND, show_shortcuts_menuitem_label.data());
                }
                if (!h_sub_menu)
                {
