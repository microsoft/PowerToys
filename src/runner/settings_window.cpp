        ...
        if (name == L"general")
        {
            try
            {
                ...
                else if (action == L"restart_maintain_elevation")
                {
                    ...
                    // since a normal PostQuitMessage(0) would usually cause this procsess to save it's in memory settings to disk, we need to 
                    // send a PostQuitMessage(1) and chaeck for that on exit, and skip the settings-flush.
