...
bool FancyZones::ShouldProcessNewWindow(HWND window) noexcept
{
    ...
    if (IsSplashScreen(window) ||
        (reinterpret_cast<size_t>(::GetProp(window, ZonedWindowProperties::MultipleZone)) != 0) ||
...
void FancyZones::UpdateWindowsPositions() noexcept
{
    auto callback = [](HWND window, LPARAM data) -> BOOL {
        size_t bitmask = reinterpret_cast<size_t>(::GetProp(window, ZonedWindowProperties::MultipleZone));
