...
{
    // r3-p3-01: a loadMore that fails with a JSON-RPC error must not strand the
    ...
    // r3-p3-04: the initialize handshake id, displayName and icon are applied to
    ...
    public void Provider_HandshakeIdentityAndIconAreApplied()
    {
        ...
        var provider = new JSCommandProviderProxy(fake.Connection, Manifest(), metadata);
    ...
    // r3-p3-04: when the handshake omits the identity fields the provider falls
    // back to the static package manifest values.
    [TestMethod]
    public void Provider_HandshakeMissingFieldsFallBackToManifest()
    {
        using var fake = new JSFakeExtension();
        var provider = new JSCommandProviderProxy(fake.Connection, Manifest());
    ...
    // r3-p3-06: a single supplied theme variant maps to both themes so the icon
    ...
    // r3-p3-06: a single dark variant mirrors to the light theme too.
    [TestMethod]
    ...
    // r3-p3-06: when both variants are supplied they are preserved independently
