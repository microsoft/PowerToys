...
internal abstract partial class WidgetPage : OnLoadContentPage
{
    ...
    protected static ILogger Logger { get; } = new CmdPalLogger();
    ...
    protected string GetTemplateForPage(WidgetPageState page)
    {
        ...
        {
            Log_UsingCachedTemplate(Logger, page);
            return value;
        }
        ...
        try
        {
            ...
            Log_CachingTemplate(Logger, page);
            Template[page] = template;
            return template;
        }
        catch (Exception e)
        {
            Log_ErrorGettingTemplate(Logger, e);
            return string.Empty;
        }
    }
    ...
    /// active. When either is activated, we will start updating. When both are
    /// removed, we will stop updating.
    /// </summary>
    ...
    [LoggerMessage(Level = LogLevel.Debug, Message = "Using cached template for {page}")]
    static partial void Log_UsingCachedTemplate(ILogger logger, WidgetPageState page);

    [LoggerMessage(Level = LogLevel.Debug, Message = "Caching template for {page}")]
    static partial void Log_CachingTemplate(ILogger logger, WidgetPageState page);

    [LoggerMessage(Level = LogLevel.Error, Message = "Error getting template.")]
    static partial void Log_ErrorGettingTemplate(ILogger logger, Exception ex);
}
...
internal sealed partial class SystemCPUUsageWidgetPage : WidgetPage, IDisposable
{
    ...
    private static string SpeedToString(float cpuSpeed)
    {
...
internal sealed partial class SystemMemoryUsageWidgetPage : WidgetPage, IDisposable
{
    ...
    private static string MemUlongToString(ulong memBytes)
    {
        if (memBytes < 1024)
        {
...
internal sealed partial class SystemNetworkUsageWidgetPage : WidgetPage, IDisposable
{
    ...
    private static string BytesToBitsPerSecString(float value)
    {
        // Bytes to bits
        value *= 8;
...
internal sealed partial class OpenTaskManagerCommand : InvokableCommand
{
    ...
    private static readonly ILogger _logger = new CmdPalLogger();
    ...
    public override string Name => Resources.GetResource("Open_Task_Manager_Title", _logger);
    ...
    public override ICommandResult Invoke()
    {
        ...
        {
            Log_ErrorLaunchingTaskManager(_logger, e);
        }

        return CommandResult.Hide();
    }

    [LoggerMessage(Level = LogLevel.Error, Message = "Error launching Task Manager.")]
    static partial void Log_ErrorLaunchingTaskManager(ILogger logger, Exception ex);
}
