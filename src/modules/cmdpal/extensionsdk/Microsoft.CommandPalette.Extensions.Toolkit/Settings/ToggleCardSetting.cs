...
public sealed class ToggleCardSetting : Setting<bool>
{
    private ToggleCardSetting()
        : base()
    {
    }

    public ToggleCardSetting(string key, bool defaultValue)
        : base(key, defaultValue)
    {
    }

    public ToggleCardSetting(string key, string label, string description, bool defaultValue)
    ...
    public override Dictionary<string, object> ToDictionary()
    {
        return new()
        {
            { "type", "SettingsCard.Input.Toggle" },
            { "label", string.Empty },
            { "id", Key },
            { "header", Label },
    ...
    public static ToggleCardSetting LoadFromJson(JsonObject jsonObject) => new() { Value = jsonObject["value"]?.GetValue<bool>() ?? false };
