...
async function cmdInstall(options) {
    ...
    console.log('');
    const result = await (0, pipeline_1.installRaycastExtension)({
    ...
    for (const stage of result.stages) {
        const icon = stage.status === 'success'
            ...
            : stage.status === 'failed'
                ? '✗'
                : '○';
        const duration = stage.duration > 0 ? ` (${(stage.duration / 1000).toFixed(1)}s)` : '';
    ...
    if (result.success) {
        ...
        if (result.error && /rate.?limit/i.test(result.error)) {
            if (!options.githubToken && !process.env.GITHUB_TOKEN) {
                console.error('');
                console.error('Hint: Set a GITHUB_TOKEN environment variable to increase the rate limit from 60 to 5,000 requests/hour.');
                console.error('  Create a token at: https://github.com/settings/tokens (public_repo scope)');
            }
        }
        else if (result.error && /404|not found/i.test(result.error)) {
            console.error('');
            console.error(`Hint: Extension "${options.extensionName}" may not exist. Check https://github.com/raycast/extensions/tree/main/extensions`);
        }
        process.exit(1);
    }
}
function cmdUninstall(options) {
    ...
    const removed = (0, manage_1.uninstallExtension)(options.extensionName, options.outputDir);
    if (removed) {
...
function cmdList(options) {
    const extensions = (0, manage_1.listInstalledExtensions)(options.outputDir);
    if (extensions.length === 0) {
        console.log('No Raycast-compat extensions installed.');
        const dir = options.outputDir ?? (0, stage_install_1.getDefaultInstallDir)();
