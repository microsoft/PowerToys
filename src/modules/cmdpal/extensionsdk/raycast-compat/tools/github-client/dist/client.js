...
exports.RaycastExtensionsClient = exports.GitHubApiError = exports.RateLimitError = void 0;
/**
...
class RaycastExtensionsClient {
    ...
    constructor(options = {}) {
        ...
        this.cache = new cache_1.Cache(options.cacheTtlMs ?? DEFAULT_CACHE_TTL_MS);
    }
    ...
    async listExtensionsViaContents() {
        const data = await this.request(`/repos/${this.owner}/${this.repo}/contents/${EXTENSIONS_DIR}`);
    ...
    async listExtensionsViaTree() {
        // Get the default branch SHA, then walk the tree
        const refData = await this.request(`/repos/${this.owner}/${this.repo}/git/ref/heads/main`);
        const commitSha = refData.object.sha;
        // Get the root tree
        const rootTree = await this.request(`/repos/${this.owner}/${this.repo}/git/trees/${commitSha}`);
        const extensionsTree = rootTree.tree.find((e) => e.path === EXTENSIONS_DIR && e.type === 'tree');
        if (!extensionsTree) {
        ...
        const subTree = await this.request(`/repos/${this.owner}/${this.repo}/git/trees/${extensionsTree.sha}`);
        return subTree.tree
     ...
     */
    async getManifest(extensionName) {
        const cacheKey = `manifest:${extensionName}`;
        const cached = this.cache.get(cacheKey);
        if (cached !== undefined)
            return cached;
        try {
            const file = await this.request(`/repos/${this.owner}/${this.repo}/contents/${EXTENSIONS_DIR}/${extensionName}/package.json`);
    ...
    async searchExtensions(query, options = {}) {
        ...
        if (cached)
            return cached;
        const q = encodeURIComponent(`${query} repo:${this.owner}/${this.repo} path:${EXTENSIONS_DIR} filename:package.json`);
        const data = await this.request(`/search/code?q=${q}&per_page=${limit}&page=${page}`);
     ...
     */
    async getReadme(extensionName) {
        const cacheKey = `readme:${extensionName}`;
        const cached = this.cache.get(cacheKey);
        if (cached !== undefined)
            return cached;
        try {
            const file = await this.request(`/repos/${this.owner}/${this.repo}/contents/${EXTENSIONS_DIR}/${extensionName}/README.md`);
     ...
     */
    async downloadExtension(extensionName) {
        ...
        const tree = await this.request(`/repos/${this.owner}/${this.repo}/git/trees/${treeSha}?recursive=1`);
        ...
        for (let i = 0; i < blobs.length; i += batchSize) {
            ...
            const results = await Promise.all(batch.map(async (blob) => {
                try {
                    const data = await this.request(`/repos/${this.owner}/${this.repo}/git/blobs/${blob.sha}`);
     ...
     */
    async resolveTreeSha(targetPath) {
        const refData = await this.request(`/repos/${this.owner}/${this.repo}/git/ref/heads/main`);
        const parts = targetPath.split('/');
        let currentSha = refData.object.sha;
        // Walk from root commit → each path segment
        // First get the root tree from the commit
        const commit = await this.request(`/repos/${this.owner}/${this.repo}/git/commits/${currentSha}`);
        currentSha = commit.tree.sha;
        for (const part of parts) {
            const tree = await this.request(`/repos/${this.owner}/${this.repo}/git/trees/${currentSha}`);
     ...
     */
    async filterWindowsCompatible(entries) {
        const result = [];
        const batchSize = 10;
        for (let i = 0; i < entries.length; i += batchSize) {
            const batch = entries.slice(i, i + batchSize);
            const manifests = await Promise.all(batch.map((e) => this.getManifest(e.name)));
...
exports.RaycastExtensionsClient = RaycastExtensionsClient;
