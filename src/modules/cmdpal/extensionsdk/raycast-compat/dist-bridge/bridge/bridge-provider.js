...
class RaycastBridgeProvider {
     ...
     */
    _inferGridPropertiesFromManifest() {
        const prefs = this._manifest.preferences ?? [];
        const gridPref = prefs.find((p) => p.name.toLowerCase().includes('griditemsize') ||
        ...
        const prefDefault = gridPref.default ?? 'medium';
        const layoutMap = {
            small: 'small',
            medium: 'medium',
            large: 'gallery',
    ...
    _invokeItemAction(itemIndex, actionIndex) {
        ...
        if (action.type === 'Action.Open' || action.type === 'Action.OpenInBrowser') {
            ...
            const target = (action.props.url ?? action.props.target);
            if (target) {
                Promise.resolve().then(() => __importStar(require('../api-stubs/navigation'))).then((nav) => nav.open(target)).catch((err) => {
        ...
        if (action.type === 'Action.CopyToClipboard') {
            ...
            if (text) {
                Promise.resolve().then(() => __importStar(require('../api-stubs/clipboard'))).then((clip) => clip.Clipboard.copy(text)).catch((err) => {
        ...
        if (action.type === 'Action.Paste') {
            const content = action.props.content;
            if (content) {
                Promise.resolve().then(() => __importStar(require('../api-stubs/clipboard'))).then((clip) => clip.Clipboard.paste(content)).catch((err) => {
        ...
        for (const child of listNode.children) {
            ...
            if (child.type === 'List.Item' || child.type === 'Grid.Item') {
                ...
                for (const sectionChild of child.children) {
                    if ((0, vnode_1.isElementVNode)(sectionChild) && (sectionChild.type === 'List.Item' || sectionChild.type === 'Grid.Item')) {
