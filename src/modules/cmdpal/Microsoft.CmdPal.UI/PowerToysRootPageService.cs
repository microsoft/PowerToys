...
internal sealed class PowerToysRootPageService : IRootPageService
{
    ...
    public void OnPerformAnyCommand(IExtensionHost? host, object? senderContext)
    {
        var extensionHost = host as CommandPaletteHost;
        var extension = extensionHost?.Extension ?? null;

        if (extension == null && senderContext is TopLevelViewModel topLevelViewModel)
        {
