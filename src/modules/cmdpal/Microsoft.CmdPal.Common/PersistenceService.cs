...
public partial class PersistenceService
{
    ...
    public PersistenceService(IApplicationInfoService applicationInfoService, Microsoft.Extensions.Logging.ILogger logger)
    {
        ...
        Action<T>? afterWriteCallback)
    {
        if (string.IsNullOrEmpty(fileName))
        {
            throw new InvalidOperationException($"You must set a valid file name before saving {typeof(T).Name}");
        }
    ...
    {
        var configDirectory = _applicationInfoService.ConfigDirectory;
