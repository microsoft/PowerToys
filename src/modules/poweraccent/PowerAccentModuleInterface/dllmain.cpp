...
const static wchar_t* MODULE_NAME = L"KeyboardAccent";
const static wchar_t* MODULE_DESC = L"A module that keeps your computer KeyboardAccent on-demand.";
...
private:
    ...
    void launch_process()
    {
        Logger::trace(L"Launching PowerToys KeyboardAccent process");
        ...
        Logger::trace(L"PowerToys KeyboardAccent launching: " + full_command_path);
        ...
        if (!CreateProcess(application_path.c_str(), full_command_path.data(), NULL, NULL, true, NULL, NULL, NULL, &info, &p_info))
        {
            ...
            std::wstring message = L"PowerToys KeyboardAccent failed to start with error: ";
...
public:
    PowerAccent()
    {
        ...
        LoggerHelpers::init_logger(app_key, L"ModuleInterface", "KeyboardAccent");
    ...
    virtual void disable()
    {
        if (m_enabled)
        {
            Logger::trace(L"Disabling KeyboardAccent... {}", m_enabled);
            ResetEvent(m_hInvokeEvent);

            auto exitEvent = CreateEvent(nullptr, false, false, CommonSharedConstants::POWERACCENT_EXIT_EVENT);
            if (!exitEvent)
            {
                Logger::warn(L"Failed to create exit event for PowerToys KeyboardAccent. {}", get_last_error_or_default(GetLastError()));
            }
            else
            {
                Logger::trace(L"Signaled exit event for PowerToys KeyboardAccent.");
                if (!SetEvent(exitEvent))
                {
                    Logger::warn(L"Failed to signal exit event for PowerToys KeyboardAccent. {}", get_last_error_or_default(GetLastError()));
