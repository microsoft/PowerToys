...
namespace winrt::PowerToys::PowerAccentKeyboardService::implementation
{
    ...
    void KeyboardListener::InitHook()
    {
        ...
        if (!hook_disabled)
        {
            ...
            // Cancel the accent gesture when the foreground moves away from the window
            // associated with the toolbar. This also recovers a toolbar left visible
            // after a missed key release. Skip events generated by this process.
    ...
    void KeyboardListener::ForceReset()
    {
        ...
        m_toolbarForegroundWindow = nullptr;
    ...
    bool KeyboardListener::OnKeyDown(KBDLLHOOKSTRUCT info) noexcept
    {
            ...
            !IsForegroundAppExcluded())
        {
            ...
            m_triggeredWithLeftArrow = false;
            m_triggeredWithRightArrow = false;
            m_toolbarForegroundWindow = GetForegroundWindow();
    ...
    void CALLBACK KeyboardListener::ForegroundEventProc(HWINEVENTHOOK, DWORD event, HWND hwnd, LONG idObject, LONG, DWORD, DWORD) noexcept
    {
        ...
        if (s_instance == nullptr || !s_instance->m_toolbarVisible)
        {
            return;
        }

        if (!hwnd || hwnd != GetForegroundWindow() || hwnd == s_instance->m_toolbarForegroundWindow)
        {
            return;
        }
