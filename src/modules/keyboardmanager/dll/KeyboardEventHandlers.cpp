...
namespace KeyboardEventHandlers
{
    ...
    {
        // Num Lock's key state is applied before it is intercepted by low level keyboard hooks, so we have to manually set back the state when we suppress the key. This is done by send an addition key up, down set of messages.
