...
namespace RemappingLogicTests
{
    ...
    public:
        ...
        TEST_METHOD (SetKeyEvent_ShouldUseExtendedKeyFlag_WhenArgumentIsExtendedKey)
        {
            ...
            const auto keyCodes = std::array{ VK_RCONTROL, VK_RMENU, VK_NUMLOCK, VK_SNAPSHOT, VK_CANCEL, VK_INSERT, VK_HOME, VK_PRIOR, VK_DELETE, VK_END, VK_NEXT, VK_LEFT, VK_DOWN, VK_RIGHT, VK_UP };

            for (int i = 0; i < keyCodes.size(); i++)
            {
                // Set key events for all the extended keys
                Helpers::SetKeyEvent(inputs, INPUT_KEYBOARD, keyCodes.at(i), 0, 0);
                // Extended key flag should be set
                Assert::AreEqual(static_cast<DWORD>(KEYEVENTF_EXTENDEDKEY), inputs.at(i).ki.dwFlags & KEYEVENTF_EXTENDEDKEY);
            }
        }
        ...
        TEST_METHOD (SetKeyEvent_ShouldSetScanCodeFieldTo0_WhenArgumentIsDummyKey)
        {
            ...
            Assert::AreEqual(static_cast<WORD>(0), inputs[0].ki.wScan);
            Assert::AreEqual(static_cast<WORD>(0), inputs[1].ki.wScan);
        }
    };
}
