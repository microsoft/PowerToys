...
constexpr auto IsKeyDown(WPARAM wParam)
{
...
namespace KeyboardEventHandlers
{
    ...
    intptr_t HandleSingleKeyRemapEvent(KeyboardManagerInput::InputInterface& ii, LowlevelKeyboardEvent* data, State& state) noexcept
    {
        ...
        {
            const auto originalKeyCode = data->lParam->vkCode;
            const auto remapResult = state.GetSingleKeyRemapResult(originalKeyCode, IsKeyDown(data->wParam));
            for (const auto& keyEvent : remapResult.remappedKeyEvents)
            {
                if (IsValidSingleKeyOrShortcut(keyEvent.key))
                {
                    // Check if the remap is to a key or a shortcut
                    bool remapToKey = (keyEvent.key.index() == 0);

                    // If mapped to VK_DISABLED then the key is disabled
                    if (remapToKey)
                    {
                        if (std::get<DWORD>(keyEvent.key) == CommonSharedConstants::VK_DISABLED)
                        {
                            continue;
                        }
                    }
                    ...
                    if (remapToKey)
                    {
                        target = Helpers::FilterArtificialKeys(std::get<DWORD>(keyEvent.key));
                    }
                    else
                    {
                        target = Helpers::FilterArtificialKeys(std::get<Shortcut>(keyEvent.key).GetActionKey());
                    }
                    ...
                    {
                        ResetIfModifierKeyForLowerLevelKeyHandlers(ii, originalKeyCode, target);
                    }

                    if (remapToKey)
                    {
                        // Inject a remapped key.
                        const auto flags = keyEvent.isKeyDown ? 0 : KEYEVENTF_KEYUP;
                        Helpers::SetKeyEvent(keyEventList, INPUT_KEYBOARD, static_cast<WORD>(target), flags, KeyboardManagerConstants::KEYBOARDMANAGER_SINGLEKEY_FLAG);
                    }
                    else
                    {
                        const auto& targetShortcut = std::get<Shortcut>(keyEvent.key);
                        if (keyEvent.isKeyDown)
                        {
                    ...
                    if (keyEvent.isKeyDown)
                    {
                        ...
                        if (remapToKey)
                        {
                            ResetIfModifierKeyForLowerLevelKeyHandlers(ii, target, originalKeyCode);
                        }
                        else
                        {
                            std::vector<DWORD> shortcutKeys = std::get<Shortcut>(keyEvent.key).GetKeyCodes();
                            for (auto& itSk : shortcutKeys)
                            {
                                ResetIfModifierKeyForLowerLevelKeyHandlers(ii, itSk, originalKeyCode);
                            }
                        }
                    }
                }
            ...
            return remapResult.shouldEatOriginalKey ? 1 : 0;
        }
                ...
                if (data->lParam->vkCode == it->first.GetActionKey() && (data->wParam == WM_KEYDOWN || data->wParam == WM_SYSKEYDOWN))
                {
                    ...
                    if (remapToShortcut)
                    {
                        ...
                        if (commonKeys == src_size - 1)
                        {
                            ...
                            Helpers::SetKeyEvent(keyEventList, INPUT_KEYBOARD, (WORD)std::get<Shortcut>(it->second.targetShortcut).GetActionKey(), 0, KeyboardManagerConstants::KEYBOARDMANAGER_SHORTCUT_FLAG);
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        if (std::get<DWORD>(it->second.targetShortcut) != CommonSharedConstants::VK_DISABLED)
                        {
                            Helpers::SetKeyEvent(keyEventList, INPUT_KEYBOARD, (WORD)Helpers::FilterArtificialKeys(std::get<DWORD>(it->second.targetShortcut)), 0, KeyboardManagerConstants::KEYBOARDMANAGER_SHORTCUT_FLAG);
                        }
            ...
            else if (it->second.isShortcutInvoked)
            {
                ...
                if ((it->first.CheckWinKey(data->lParam->vkCode) || it->first.CheckCtrlKey(data->lParam->vkCode) || it->first.CheckAltKey(data->lParam->vkCode) || it->first.CheckShiftKey(data->lParam->vkCode)) && (data->wParam == WM_KEYUP || data->wParam == WM_SYSKEYUP))
                {
                    ...
                    if (remapToShortcut)
                    {
                        ...
                        {
                            Helpers::SetKeyEvent(keyEventList, INPUT_KEYBOARD, (WORD)std::get<Shortcut>(it->second.targetShortcut).GetActionKey(), KEYEVENTF_KEYUP, KeyboardManagerConstants::KEYBOARDMANAGER_SHORTCUT_FLAG);
                        }
                    ...
                    else
                    {
                        ...
                        {
                            Helpers::SetKeyEvent(keyEventList, INPUT_KEYBOARD, (WORD)Helpers::FilterArtificialKeys(std::get<DWORD>(it->second.targetShortcut)), KEYEVENTF_KEYUP, KeyboardManagerConstants::KEYBOARDMANAGER_SHORTCUT_FLAG);
                        }
                    ...
                    if (data->wParam == WM_KEYDOWN || data->wParam == WM_SYSKEYDOWN)
                    {
                        if (remapToShortcut)
                        {
                            ...
                            if (newRemappingIter != reMap.end())
                            {
                                ...
                                if (newRemapping.RemapToKey())
                                {
                                    ...
                                    if (ii.GetVirtualKeyState((WORD)from.actionKey))
                                    {
                                        ...
                                        Helpers::SetKeyEvent(keyEventList, INPUT_KEYBOARD, (WORD)from.actionKey, KEYEVENTF_KEYUP, KeyboardManagerConstants::KEYBOARDMANAGER_SHORTCUT_FLAG);
                                    }
                                    Helpers::SetKeyEvent(keyEventList, INPUT_KEYBOARD, (WORD)to, 0, KeyboardManagerConstants::KEYBOARDMANAGER_SHORTCUT_FLAG);
                                }
                                else
                                {
                                    ...
                                    Helpers::SetKeyEvent(keyEventList, INPUT_KEYBOARD, (WORD)to.actionKey, 0, KeyboardManagerConstants::KEYBOARDMANAGER_SHORTCUT_FLAG);
                            ...
                            else
                            {
                                ...
                                if (isActionKeyPressed)
                                {
                                    Helpers::SetKeyEvent(keyEventList, INPUT_KEYBOARD, (WORD)it->first.GetActionKey(), 0, KeyboardManagerConstants::KEYBOARDMANAGER_SHORTCUT_FLAG);
                                }
                                ...
                                Helpers::SetKeyEvent(keyEventList, INPUT_KEYBOARD, (WORD)data->lParam->vkCode, 0, 0);
    ...
    void ResetIfModifierKeyForLowerLevelKeyHandlers(KeyboardManagerInput::InputInterface& ii, DWORD key, DWORD target)
    {
        ...
        if (target == VK_CAPITAL)
        {
            ...
            if (Helpers::IsModifierKey(key) && !(key == VK_LWIN || key == VK_RWIN || key == CommonSharedConstants::VK_WIN_BOTH))
            {
                ...
                Helpers::SetKeyEvent(keyEventList, INPUT_KEYBOARD, (WORD)key, KEYEVENTF_KEYUP, KeyboardManagerConstants::KEYBOARDMANAGER_SUPPRESS_FLAG);
                ii.SendVirtualInput(keyEventList);
            }
        }
    }
