            ...
            CancellationToken cancellationToken)
        {
            try
            {
                ...
                for (int i = 0; i < physicals.Length; i++)
                {
                    ...
                    // first syscall. If the call BSODs (kernel stack-cookie overrun inside
                    // win32kfull!DdcciGetCapabilitiesStringFromMonitor — see GH #47556 / #47968),
                    // this is the last log line that survives, so it has to carry enough to
                    // identify the offending hardware: EdidId for blacklist matching, plus the
                    // human-readable name and full DevicePath.
