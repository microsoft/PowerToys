...
{
    /// <summary>
    /// Basic monitor controller contract — what every controller (DDC/CI, WMI, ...) must provide.
    /// Covers monitor discovery and brightness (the one feature both DDC/CI and the Windows
    /// <c>WmiMonitorBrightness</c> path can deliver). DDC/CI-only extras (contrast, volume,
    /// color temperature, input source, power state) live on <see cref="IDdcController"/>.
    /// </summary>
    /// <remarks>
