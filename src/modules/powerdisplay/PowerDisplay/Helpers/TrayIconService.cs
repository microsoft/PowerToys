    ...
    internal sealed partial class TrayIconService
    {
        ...
        private void HandleTrayMouseMove()
        {
            if (!GetCursorPos(out var cursor))
            {
                ...
                {
                    Logger.LogWarning("[TrayHover] GetCursorPos failed while presenting tray hover");
            ...
            // The Shell repeats this message for every pixel of movement across the icon, so serve
            // a still-fresh rectangle from the cache instead of asking the Shell each time.
            ...
            if (!bounds.Contains(cursor.X, cursor.Y))
            {
                ...
                {
                    Logger.LogInfo(
                        $"[TrayHover] Tray hover at ({cursor.X}, {cursor.Y}) is outside the reported icon rectangle ({bounds.Left}, {bounds.Top}, {bounds.Right}, {bounds.Bottom})");
        ...
        private unsafe bool TryGetCurrentIconBounds(out TrayIconBounds bounds)
        {
            if (!TryQueryTrayIconBounds(out bounds, out var result))
            {
                ...
                else if (_isTrayIconRegistered && _hwnd != 0 && _trayIconData is not null)
                {
                    ...
                    {
                        Logger.LogWarning(
                            $"[TrayHover] Shell_NotifyIconGetRect failed with HRESULT 0x{result:X8}");
                        _boundsFailureLogged = true;
                    }
                }
        ...
        private void OnFeedbackTimerTick(DispatcherQueueTimer sender, object args)
        {
                ...
                !bounds.Contains(cursor.X, cursor.Y))
            {
                ...
                // mouse-move path will not report.
                StopHoverFeedback();
                _cachedBounds = null;
                _boundsCacheTimestamp = 0;
                return;
        ...
        /// <summary>
        /// Replaces the hover text with a transient readout - what a caller wants attached to the
        /// tray icon right now - or clears it back to the app name when <paramref name="text"/> is
        /// blank. The readout expires on its own after a couple of seconds.
        /// <para>Ignored unless the pointer is currently over the icon: the overlay is anchored to
        /// the icon rectangle, so there is nowhere meaningful to put it otherwise.</para>
        /// <para>This is the seam the tray mouse-wheel feature builds on. Formatting the text is
        /// deliberately the caller's job, so the wording and its localized resources stay with
        /// whatever gesture produced them rather than leaking into the tray plumbing.</para>
        /// </summary>
        /// <param name="text">The readout to show, or <see langword="null"/> to clear it.</param>
        internal void UpdateAdjustmentFeedback(string? text)
        {
            var now = Environment.TickCount64;
            if (!GetCursorPos(out var cursor) ||
                !TryQueryTrayIconBounds(out var bounds, out _) ||
        ...
        private void ShowTrayContextMenu(int x, int y)
        {
            ...
            // pointer produces no further tray mouse-move, so re-evaluate the hover rather than
            // leaving the icon with no hover text until the user nudges the mouse.
            HandleTrayMouseMove();
        }

        private void ActivateFromTrayIcon()
        {
            // Take the overlay down the way the Shell tooltip used to disappear on click.
            StopHoverFeedback();
