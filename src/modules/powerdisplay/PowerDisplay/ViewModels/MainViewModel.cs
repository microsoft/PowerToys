...
public class MainViewModel : INotifyPropertyChanged, IDisposable
{
        ...
        try
        {
            ...
            _dispatcherQueue.TryEnqueue(() =>
            {
                ...
                if (monitors.Count > 0)
                {
                    ...
                    // Save monitor info to settings file periodically
                    _ = Task.Run(async () => await _settingsManager.SaveMonitorInfoPeriodicallyAsync(monitors, _cancellationTokenSource.Token));
                }
                else
                {
                    StatusText = "No controllable monitors found";

                    // Save empty monitor list to settings file
                    _ = Task.Run(async () => await _settingsManager.SaveMonitorInfoAsync(new List<Monitor>()));
                }
            });
        }
        catch (Exception ex)
    ...
    public async Task RefreshMonitorsAsync()
    {
        ...
        try
        {
            ...
            _dispatcherQueue.TryEnqueue(() =>
            {
                ...
                // Save updated monitor info to settings file
                _ = Task.Run(async () => await _settingsManager.SaveMonitorInfoAsync(monitors));
            });
        }
        catch (Exception ex)
        {
        ...
        foreach (var monitor in monitors)
        {
            ...
            if (monitor.SupportsColorTemperature && monitor.Type == MonitorType.External)
            {
                _ = Task.Run(async () =>
                {
                    try
                    {
                        await _monitorManager.InitializeColorTemperatureAsync(monitor.Id);
                        
                        // Update UI on dispatcher thread
                        _ = _dispatcherQueue.TryEnqueue(() =>
                        {
                            vm.UpdateFromModel(monitor);
                        });
                    }
                    catch (Exception ex)
                    {
    ...
    private void OnMonitorsChanged(object? sender, MonitorListChangedEventArgs e)
    {
        _dispatcherQueue.TryEnqueue(() =>
        {
            ...
            // Save updated monitor info to settings file
            _ = Task.Run(async () => await _settingsManager.SaveMonitorInfoAsync(e.AllMonitors));
        });
    }
    ...
    public void ReloadMonitorSettings()
    {
        try
        {
            ...
            else
            {
                ...
                _ = _settingsManager.FlushPendingChangesAsync();
    ...
    public void SaveMonitorSetting(string monitorId, string property, int value)
    {
        try
        {
            ...
            // Queue the setting change for batch processing to avoid file conflicts
            // Note: This is one-way save only - we don't read these values back during runtime
            _settingsManager.QueueSettingChange(internalName, property, value);
