...
int WINAPI wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ PWSTR lpCmdLine, _In_ int nCmdShow)
{
    ...
    {
        auto mainThreadId = GetCurrentThreadId();
        EventWaiter exitEventWaiter = EventWaiter(CommonSharedConstants::SHORTCUT_GUIDE_EXIT_EVENT,
                                                  [mainThreadId, &window](int err)
                                                  {
                                                      if (err != ERROR_SUCCESS)
                                                      {
                                                          Logger::error(L"Failed to wait for {} event. {}",
                                                                        CommonSharedConstants::SHORTCUT_GUIDE_EXIT_EVENT,
                                                                        get_last_error_or_default(err));
                                                      }
                                                      else
                                                      {
                                                          Logger::trace(L"{} event was signaled",
                                                                        CommonSharedConstants::SHORTCUT_GUIDE_EXIT_EVENT);
                                                      }

                                                      window.CloseWindow(HideWindowType::THE_SHORTCUT_PRESSED,
                                                                         mainThreadId);
                                                  });
    }
