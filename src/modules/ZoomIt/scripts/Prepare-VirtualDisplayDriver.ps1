...
<#
.SYNOPSIS
Prepares the pinned external driver package used by the local ZoomIt virtual mirror experiment.
.DESCRIPTION
...
if (-not [Environment]::Is64BitOperatingSystem -or $env:PROCESSOR_ARCHITECTURE -eq 'ARM64' -or $env:PROCESSOR_ARCHITEW6432 -eq 'ARM64') {
    throw 'This pinned experimental package supports Windows x64 only.'
}
...
Write-Host 'Shared driver settings are not copied; ZoomIt virtual mirror uses the built-in one-monitor defaults.'
...
if ($Stage) {
    ...
    Write-Host 'Driver package staged. No display device was created. Launch the experimental ZoomIt build as administrator to create its temporary display.'
} else {
