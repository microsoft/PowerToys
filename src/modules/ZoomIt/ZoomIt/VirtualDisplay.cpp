...
namespace
{
    ...
    constexpr DWORD MaximumDisplayDimension = 16384;
    constexpr char StagingHelp[] = " The supported Virtual Display Driver package must already be staged in Windows. Restart PowerToys or ZoomIt as administrator.";
    ...
    void WaitForRetiredInstance(std::wstring& retiredInstance, const std::atomic_bool& cancel)
    {
        ...
        for (;;)
        {
            ...
            {
                throw std::runtime_error("Windows is still removing the previous ZoomIt display. Wait a moment before starting another session.");
            }
    ...
    void RequireUnusedDriverDefaults()
    {
        ...
        for (DWORD index = 0; SetupDiEnumDeviceInfo(devices.value, index, &deviceInfo); ++index)
        {
            ...
            for (const wchar_t* id = ids.data(); *id; id += wcslen(id) + 1)
            {
                ...
                {
                    throw std::runtime_error("An existing Virtual Display Driver device is present. ZoomIt virtual mirroring requires its own temporary adapter and will not change an existing adapter.");
                }
            }
        }
        ...
        if (RegOpenKeyExW(HKEY_LOCAL_MACHINE, L"SOFTWARE\\MikeTheTech\\VirtualDisplayDriver", 0, KEY_QUERY_VALUE, &key) == ERROR_SUCCESS)
        {
            ...
            {
                throw std::runtime_error("An existing Virtual Display Driver configuration override was found. ZoomIt virtual mirroring requires the driver's unmodified defaults and will not replace shared settings.");
            }
        }
        for (const auto* path : { L"C:\\VirtualDisplayDriver\\vdd_settings.xml", L"C:\\VirtualDisplayDriver\\option.txt" })
        {
            ...
            {
                throw std::runtime_error("An existing Virtual Display Driver configuration file was found. ZoomIt virtual mirroring requires the driver's built-in one-monitor defaults and will not replace shared settings.");
            }
        }
    }
    ...
    bool IsOwnedInstance(const wchar_t* instance, const std::wstring& ownedInstance)
    {
        ...
        DEVINST device = 0;
        std::wstring instanceId(instance);
        return CM_Locate_DevNodeW(&device, instanceId.data(), CM_LOCATE_DEVNODE_NORMAL) == CR_SUCCESS && IsOwnedDeviceOrAncestor(device, ownedInstance);
    }
    ...
    bool ApplySupportedMode(const std::wstring& deviceName, int requestedWidth, int requestedHeight, LONG desktopRight, LONG desktopTop, Clock::time_point deadline, const std::atomic_bool& cancel)
    {
        ...
        for (DWORD index = 0; index != 4096; ++index)
        {
            ...
            if (mode.dmPelsWidth && mode.dmPelsHeight && mode.dmPelsWidth <= MaximumDisplayDimension && mode.dmPelsHeight <= MaximumDisplayDimension)
            {
        ...
        for (auto mode : modes)
        {
            ...
            if (static_cast<int64_t>(desktopRight) + mode.dmPelsWidth > (std::numeric_limits<LONG>::max)() ||
                static_cast<int64_t>(desktopTop) + mode.dmPelsHeight > (std::numeric_limits<LONG>::max)())
            {
                continue;
            }
...
namespace zoomit_mirror
{
    ...
    bool VirtualDisplay::Create(int requestedWidth, int requestedHeight, std::wstring& error, const std::atomic_bool& cancel)
    {
        error.clear();
        try
        {
            CreateInternal(requestedWidth, requestedHeight, cancel);
            return true;
        }
        catch (const std::exception& exception)
        {
            Close();
            // These diagnostics are ASCII; expose them as UTF-16 for ZoomIt's UI.
        ...
        {
            Close();
            error = L"An unexpected error occurred while creating the temporary virtual display.";
        }
        return false;
    }

    void VirtualDisplay::CreateInternal(int requestedWidth, int requestedHeight, const std::atomic_bool& cancel)
    {
        Close();
        if (requestedWidth <= 0 || requestedHeight <= 0 ||
            static_cast<DWORD>(requestedWidth) > MaximumDisplayDimension || static_cast<DWORD>(requestedHeight) > MaximumDisplayDimension)
        {
            throw std::runtime_error("The requested virtual display size is outside the supported range.");
        }
        if (!IsElevated())
        {
            throw std::runtime_error(std::string("Creating the temporary virtual display requires an elevated ZoomIt process.") + StagingHelp);
        }
        ...
        createInfo.pszDeviceDescription = L"PowerToys ZoomIt temporary display";
        ...
        try
        {
            ...
            const HRESULT result = SwDeviceCreate(L"PowerToysZoomItMirror", L"HTREE\\ROOT\\0", &createInfo, 0, nullptr, DeviceCreated, &completion, &m_device);
            if (FAILED(result))
            {
            ...
            throw std::runtime_error(std::string("Timed out waiting for the owned virtual display and a supported display mode. Check that the supported driver package for this PC is staged and accepted by Windows.") + diagnostic + StagingHelp);
        }
        catch (...)
        {
