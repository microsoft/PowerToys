...
{
    /// <summary>
    /// Unlocks the Phi Silica Limited Access Feature. Mirrors Advanced Paste's helper of the same
    /// name: the feature id and attestation wording are dictated by the LAF registration, so both
    /// modules must request it identically.
    /// </summary>
    internal static class PhiSilicaLafHelper
    {
        ...
        public static bool TryUnlock()
        {
            ...
            // the next call lets the module recover without restart.
            if (_unlocked)
            {
                return true;
            }
