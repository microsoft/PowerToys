...
namespace WorkspacesLibUnitTests
{
    ...
    public:
        ...
        TEST_METHOD(ReadSingleWorkspace_InvalidJsonFile_ReturnsError)
        {
            ...
            Assert::AreEqual(static_cast<int>(JsonUtils::WorkspacesFileError::FileReadingError), 
        ...
        TEST_METHOD(ReadWorkspaces_NonExistentFile_ReturnsEmptyVector)
        {
            ...
            // Assert
            Assert::IsTrue(result.isOk());
            auto workspaces = result.value();
            Assert::AreEqual(static_cast<size_t>(0), workspaces.size());
        }
