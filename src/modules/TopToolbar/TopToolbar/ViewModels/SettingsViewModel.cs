...
namespace TopToolbar.ViewModels
{
    public class SettingsViewModel : ObservableObject, System.IDisposable
    {
        ...
        private async Task SaveCoreAsync()
        {
            ...
            AppLogger.LogInfo("SaveCoreAsync: config saved");
        }
        ...
        private static bool ShouldAutoManageIcon(ToolbarButton button, string iconsDir)
        {
            ...
            if (button.IconType == ToolbarIconType.Image)
            {
                ...
                return IsManagedImageIcon(button.IconPath, iconsDir);
            }
        ...
        private static bool IsManagedImageIcon(string iconPath, string iconsDir)
        {
            ...
            try
            {
                var iconFullPath = Path.GetFullPath(iconPath);
                var iconsFullPath = Path.GetFullPath(iconsDir);
                if (!iconsFullPath.EndsWith(Path.DirectorySeparatorChar))
                {
                    iconsFullPath += Path.DirectorySeparatorChar;
                }

                return iconFullPath.StartsWith(iconsFullPath, StringComparison.OrdinalIgnoreCase);
            }
            catch
            {
                return false;
