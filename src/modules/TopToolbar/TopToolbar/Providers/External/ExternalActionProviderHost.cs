...
namespace TopToolbar.Providers.External
{
    public sealed class ExternalActionProviderHost : IDisposable
    {
        ...
        private void OnProcessExited(object sender, EventArgs e)
        {
            lock (_sync)
            {
                Logger.LogInfo($"ExternalActionProviderHost: provider '{_providerId}' exited with code {_process?.ExitCode ?? -1}.");
                CleanupResources();
            }
        }
        ...
        private async Task PumpStandardErrorAsync(CancellationToken token)
        {
            try
            {
                ...
                while (!token.IsCancellationRequested)
                {
                    ...
                    {
                        Logger.LogInfo($"ExternalActionProviderHost[{_providerId}]: {line}");
                    }
                }
            }
            ...
            {
                Logger.LogWarning($"ExternalActionProviderHost: stderr pump for '{_providerId}' failed - {ex.Message}.");
            }
        }
