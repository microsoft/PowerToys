...
internal sealed class McpClient : IDisposable
{
    ...
    public async Task InitializeAsync(CancellationToken cancellationToken)
    {
        ...
        var initParams = new
        {
            ...
            clientInfo = new { name = "PowerToys.TopToolbar", version = "1.0.0" },
        };
    ...
    private async Task ReadLoopAsync(CancellationToken cancellationToken)
    {
        try
        {
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                {
                    Logger.LogWarning($"McpClient: failed to parse MCP response - {ex.Message}.");
                }
            }
        }
        ...
        {
            Logger.LogWarning($"McpClient: read loop failed - {ex.Message}.");
        }
        finally
        {
