...
public sealed class McpActionProvider : IActionProvider, IToolbarGroupProvider, IDisposable
{
    ...
    public async IAsyncEnumerable<ActionDescriptor> DiscoverAsync(ActionContext context, [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        {
            Logger.LogWarning($"McpActionProvider[{Id}]: discovery failed - {ex.Message}.");
            yield break;
        }
        ...
        {
            Logger.LogWarning($"McpActionProvider[{Id}]: failed to resolve actions for discovery - {ex.Message}.");
            yield break;
        }
    ...
    public async Task<ButtonGroup> CreateGroupAsync(ActionContext context, CancellationToken cancellationToken)
    {
        ...
        {
            Logger.LogWarning($"McpActionProvider[{Id}]: failed to build button group - {ex.Message}.");
        ...
        {
            Logger.LogWarning($"McpActionProvider[{Id}]: failed to resolve actions for group - {ex.Message}.");
            return CreateEmptyGroup();
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            Logger.LogWarning($"McpActionProvider[{Id}]: MCP error - {ex.Message} (code {ex.ErrorCode}).");
            return new ActionResult
            {
                Ok = false,
                Message = ex.Message,
        ...
        {
            Logger.LogError($"McpActionProvider[{Id}]: invoke failed - {ex.Message}.");
            return new ActionResult
            {
                Ok = false,
                Message = ex.Message,
