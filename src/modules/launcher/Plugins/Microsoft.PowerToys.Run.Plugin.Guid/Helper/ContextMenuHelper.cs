...
namespace Microsoft.PowerToys.Run.Plugin.Guid.Helper
{
    ...
    internal static class ContextMenuHelper
    {
        ...
        {
            if (result?.ContextData is not string entry)
            {
            ...
            var list = new List<ContextMenuResult>(1)
            {
                new ContextMenuResult
                {
                    ...
                    Action = _ => TryToCopyToClipBoard(entry.ToUpper()),
                    FontFamily = "Segoe MDL2 Assets",
                    Glyph = "\xE8C8",                       // E8C8 => Symbol: Copy
                    PluginName = assemblyName,
                    Title = $"{Resources.CopyCommandUpper} (Ctrl+C)",
                },
                new ContextMenuResult
                {
                    AcceleratorKey = Key.C,
                    AcceleratorModifiers = ModifierKeys.Control | ModifierKeys.Shift,
                    Action = _ => TryToCopyToClipBoard(entry.ToLower()),
                    FontFamily = "Segoe MDL2 Assets",
                    Glyph = "\xF413",                       // E8C8 => Symbol: CopyTo
                    PluginName = assemblyName,
                    Title = $"{Resources.CopyCommandLower} (Ctrl+Shift+C)",
                },
            };
