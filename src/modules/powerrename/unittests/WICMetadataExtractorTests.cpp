...
namespace WICMetadataExtractorTests
{
    ...
    TEST_CLASS(GetMetadataPathFormatTests)
    {
    public:
        TEST_METHOD(GetMetadataPathFormat_JPEG_ReturnsJPEG)
        {
            // Test that JPEG files return JPEG path format
            WICMetadataExtractor extractor;
            ...
            {
                Logger::WriteMessage(L"JPEG test file not found, skipping test");
                return;
            }

            MetadataPathFormat format = extractor.GetMetadataPathFormat(testFile);

            Assert::IsTrue(format == MetadataPathFormat::JPEG, L"JPEG file should return JPEG path format");
        }

        TEST_METHOD(GetMetadataPathFormat_HEIF_ReturnsIFD)
        {
            // Test that HEIF files return IFD path format
            WICMetadataExtractor extractor;
            ...
            // If HEIF extension is not installed, decoder creation may fail and return JPEG as default
            // We check if the file can be decoded first
            EXIFMetadata metadata;
            bool canDecode = extractor.ExtractEXIFMetadata(testFile, metadata);

            if (canDecode)
            {
                Assert::IsTrue(format == MetadataPathFormat::IFD, L"HEIF file should return IFD path format");
            }
            else
            {
                Logger::WriteMessage(L"HEIF decoding failed - HEIF Image Extensions may not be installed");
            }
        }

        TEST_METHOD(GetMetadataPathFormat_AVIF_ReturnsIFD)
        {
            // Test that AVIF files return IFD path format (AVIF uses HEIF container)
            WICMetadataExtractor extractor;
            ...
            // If AV1 extension is not installed, decoder creation may fail and return JPEG as default
            EXIFMetadata metadata;
            bool canDecode = extractor.ExtractEXIFMetadata(testFile, metadata);

            if (canDecode)
            {
                Assert::IsTrue(format == MetadataPathFormat::IFD, L"AVIF file should return IFD path format");
            }
            else
            {
                Logger::WriteMessage(L"AVIF decoding failed - AV1 Video Extension may not be installed");
            }
        }

        TEST_METHOD(GetMetadataPathFormat_NonexistentFile_ReturnsJPEG)
        {
            // Test that nonexistent files return JPEG as default
            WICMetadataExtractor extractor;
            ...
            // Default should be JPEG when decoder fails
            Assert::IsTrue(format == MetadataPathFormat::JPEG, L"Nonexistent file should return JPEG as default");
        }

        TEST_METHOD(GetMetadataPathFormat_PNG_ReturnsJPEG)
        {
            // Test that PNG files return JPEG path format (non-IFD format)
            WICMetadataExtractor extractor;

            // Try to find any PNG file in test data, or skip
            std::wstring testFile = GetTestDataPath() + L"\\test.png";

            if (!std::filesystem::exists(testFile))
            {
                // PNG test is optional - just verify non-HEIF/TIFF files return JPEG
                Logger::WriteMessage(L"PNG test file not found, skipping test");
                return;
            }

            MetadataPathFormat format = extractor.GetMetadataPathFormat(testFile);

            Assert::IsTrue(format == MetadataPathFormat::JPEG, L"PNG file should return JPEG path format");
        }

        TEST_METHOD(GetMetadataPathFormat_MultipleJPEG_Consistent)
        {
            // Test that multiple JPEG files consistently return JPEG format
            WICMetadataExtractor extractor;

            std::wstring testFile1 = GetTestDataPath() + L"\\exif_test.jpg";
            std::wstring testFile2 = GetTestDataPath() + L"\\exif_test_2.jpg";

            if (!std::filesystem::exists(testFile1) || !std::filesystem::exists(testFile2))
            {
                Logger::WriteMessage(L"JPEG test files not found, skipping test");
                return;
            }

            MetadataPathFormat format1 = extractor.GetMetadataPathFormat(testFile1);
            MetadataPathFormat format2 = extractor.GetMetadataPathFormat(testFile2);

            Assert::IsTrue(format1 == MetadataPathFormat::JPEG, L"First JPEG should return JPEG format");
            Assert::IsTrue(format2 == MetadataPathFormat::JPEG, L"Second JPEG should return JPEG format");
            Assert::IsTrue(format1 == format2, L"Both JPEG files should return same format");
        }
    };
