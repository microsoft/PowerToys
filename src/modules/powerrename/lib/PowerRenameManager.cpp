        ...
        if (pwtd)
        {
            ...
            if (WaitForSingleObject(pwtd->startEvent, INFINITE) == WAIT_OBJECT_0)
            {
                ...
                for (UINT u = 0; u < itemCount; u++)
                {
                    ...
                    if (newName == nullptr && (flags & Uppercase || flags & Lowercase || flags & Titlecase || flags & Camelcase))
                    {
                    ...
                    if (newNameToUse != nullptr && (flags & Uppercase || flags & Lowercase || flags & Titlecase || flags & Camelcase))
                    {
