...
namespace ViewModelTests
{
    ...
    public class QuickAccessLifetimeTests
    {
        ...
        {
            var general = new MemoryRepository<GeneralSettings>(new GeneralSettings());
            var keyboard = new MemoryRepository<KeyboardManagerSettings>(new KeyboardManagerSettings());
        ...
        public void QueuedAndInFlightCallbacksCannotMutateOrReattachAfterDispose()
        {
            ...
            var general = new MemoryRepository<GeneralSettings>(original);
            var keyboard = new MemoryRepository<KeyboardManagerSettings>(new KeyboardManagerSettings());
            var queue = new Queue<Action>();
            var viewModel = CreateViewModel(general, keyboard, queue);
        ...
        [TestMethod]
        public void LiveKeyboardManagerChangesKeepItemsIdentityAndLaunchCommand()
        {
            var general = new MemoryRepository<GeneralSettings>(new GeneralSettings());
        ...
        [TestMethod]
        public void DashboardUnloadReleasesOwnedSubscriptionsAndReloadReplacesItems()
        {
            var general = new MemoryRepository<GeneralSettings>(new GeneralSettings());
            var keyboard = new MemoryRepository<KeyboardManagerSettings>(new KeyboardManagerSettings());
            var queue = new Queue<Action>();
            var lifetime = new DashboardViewModelLifetime(
                () => new DashboardViewModel(general, _ => 0, CreateViewModel(general, keyboard, queue)));
        ...
        private static WeakReference[] CreateAndDisposeDashboard(
            MemoryRepository<GeneralSettings> general,
            MemoryRepository<KeyboardManagerSettings> keyboard,
            Queue<Action> queue)
        {
            var quickAccess = CreateViewModel(general, keyboard, queue);
            var dashboard = new DashboardViewModel(general, _ => 0, quickAccess);
            keyboard.Notify();
        ...
        private sealed class MemoryRepository<T> : ISettingsRepository<T>
        {
            private Action<T> _settingsChanged;

            internal MemoryRepository(T settings)
            {
