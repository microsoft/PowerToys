...
#endif
    {
...
#endif
        {
            ...
            try
            {
                ...
                if (prerelease)
                {
                    const auto body = client.request(Uri{ ALL_RELEASES_ENDPOINT });
                ...
                {
                    const auto body = client.request(Uri{ LATEST_RELEASE_ENDPOINT });
        ...
        return std::async(std::launch::async, [new_version]() -> std::optional<std::filesystem::path> {
            ...
            *installer_download_path /= new_version.installer_filename;
            ...
            for (size_t i = 0; i < MAX_DOWNLOAD_ATTEMPTS; ++i)
            {
                ...
                {
                    http::HttpClient client;
                    client.download(new_version.installer_download_url, *installer_download_path);
